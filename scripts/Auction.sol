...
contract Auction is IERC20Metadata, Mortal {
    ...
    mapping (uint256=>uint256) lastBid;
    mapping (uint256=>address) walletWinner;
    ...
    function mint(address account, uint256 amount, uint256 tokenId) public returns (bool) {
        ...
        require(amount > getLastBid(tokenId), "Bid less than the current momentary winner");
        require(auctionEnd(tokenId) == false, "Auction ended");

        myTotalSupply = myTotalSupply + amount;
        balances[account] = balances[account] + amount;
        emit Transfer(address(0), account, amount);

        walletWinner[tokenId] = account;
        lastBid[tokenId] = amount;
    ...
    function closesAuction(uint256 tokenId) public returns (bool) {
        ...
        emit BidAuctionWinner(walletWinner[tokenId], tokenId, contractAddress);
        tokenWinner[tokenId] = tokenId;
        return true;
    }

    function auctionEnd(uint256 tokenId) public view returns (bool) {
        if (tokenId == tokenWinner[tokenId]) {
            return true;
        } else {
            return false;
        }
    ...
    function getWalletWinner(uint256 tokenId) public view returns (address) {
        return walletWinner[tokenId];
    }

    function getLastBid(uint256 tokenId) public view returns (uint256) {
        return lastBid[tokenId];
    }
}
